...
class CircuitConstructor:
    ...
    def _get_controlled_gate_operations(
        ...
        # the largest index.
        if set(label) != {"I"}:
            cnot_inds = [i + len(ctrl_states) for i, c in enumerate(label) if c != "I"]
            pivot = min(cnot_inds)
            cnot_inds.remove(pivot)
            h_inds = [i + len(ctrl_states) for i, c in enumerate(label) if c == "X"]
            rx_inds = [i + len(ctrl_states) for i, c in enumerate(label) if c == "Y"]
    ...
    def build_diagonal(self, pauli_op: SparsePauliOp) -> cirq.Circuit:
        ...
        circuit.append(
            self._get_controlled_gate_operations(pauli_op[0], ctrl_states=[0])
        )
        circuit.append(
            self._get_controlled_gate_operations(pauli_op[1], ctrl_states=[1])
        )
