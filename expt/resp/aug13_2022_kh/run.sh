...
then
    ...
    do
        python3 process_results.py $h5fname kh_resp_exact kh_0813
    done

    for h5fname in kh_resp_tomo2q_raw kh_resp_tomo2q_pur kh_resp_tomo2q_trace
    do
        python3 process_results.py $h5fname kh_resp_exact kh_2q_0813
    done

    for h5fname in kh_resp_tomo_rc_raw kh_resp_tomo_rc_pur kh_resp_tomo_rc_trace
    do
        python3 process_results.py $h5fname kh_resp_exact kh_RC_0813
    done

    for h5fname in kh_resp_tomo2q_rc_raw kh_resp_tomo2q_rc_pur kh_resp_tomo2q_rc_trace
    do
        python3 process_results.py $h5fname kh_resp_exact kh_2q_RC_0813
    done
fi

if [ $generate_data = true ]
then
    python3 generate_data.py -O kh_resp_exact
    python3 generate_data.py -t kh_resp_exact
    ...
    do
        ...
        python3 generate_data.py -f kh_resp_exact $h5fname
