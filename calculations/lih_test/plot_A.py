...
omegas, lih_new_qasm = np.loadtxt('data/lih_new_qasmc_A.dat').T
#omegas, lih_new1_A = np.loadtxt('data/lih_new1_A.dat').T
#omegas, lih_eh_exact_A = np.loadtxt('data/lih_eh_exact_A.dat').T

fig, ax = plt.subplots()
#ax.plot(omegas, A_red_sv, label='Old')
ax.plot(omegas, lih_new_stat - A_red_sv, ls='--', marker='x', markevery=5, label='New (SV) - Old')
ax.plot(omegas, lih_new_qasm - A_red_sv, ls='--', marker='x', markevery=5, label='New (QASM) - Old')
...
ax.set_xlim([-20, 10])
ax.legend()
fig.savefig('figs/A_diff.png', dpi=300)
